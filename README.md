...
4. The gate of M3 is tied up to the $V_{in}$ instead of a constant DC bias voltage, so that, when $V_{DS,M2}$ moves up and down due to $V_{GS1}$ increasing and decreasing, the same happens across M4 also, which improves matching, where transistors M2 and M4 operate under identical electrical and topological conditions.
